...
class CadworkConnection:
    ...
    def send_command(self, operation: str, args: Dict[str, Any] = {}) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Encoded command ({len(command_bytes)} bytes). Attempting sendall...")
            self.sock.sendall(command_bytes)
            logger.info(f"sendall completed successfully.")
            logger.info(f"Command sent ({len(command_bytes)} bytes), waiting for response...")

            # Receive the response
            logger.info("Calling receive_full_response...")
            ...
            logger.error(f"Socket connection error ({type(e).__name__}) with Cadwork plug-in: {e}", exc_info=True)
            self.disconnect() # Disconnect on connection error
            raise ConnectionError(f"Connection to Cadwork plug-in lost: {e}")
        except json.JSONDecodeError as e:
            logger.error(f"Invalid JSON response from Cadwork plug-in: {e}", exc_info=True)
            ...
            logger.error(f"Unexpected error ({type(e).__name__}) communicating with Cadwork plug-in: {e}", exc_info=True)
...
async def server_lifespan(server: FastMCP) -> AsyncIterator[Dict[str, Any]]:
    ...
    logger.info(f"CadworkMCP server starting up. Attempting connection to plug-in at {host}:{port}...") # Log the resolved port
